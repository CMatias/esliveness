...
function computeGen(astNode) {
	...
	case 'VariableDeclarator':
		if (astNode.init) return computeGen(astNode.init);
		else return [];
...
function computeKill(astNode) {
	...
	case 'VariableDeclarator':
		if (astNode.init) return computeKill(astNode.id);
		else return [];
...
function computeLive(flowNode) {
	...
	flowNode.next.forEach(function (nextFlowNode) {
		if (!nextFlowNode.astNode || !nextFlowNode.astNode.liveness) return;
...
function compute(ast, cfg) {
	...
	(function rec(flowNode) {
		if (flowNode.type === undefined) {
			...
			if (deepEqual(prevLiveness, flowNode.astNode.liveness)) return;
			else return flowNode.prev.slice().reverse().forEach(rec);
		}
		else return flowNode.prev.slice().reverse().forEach(rec);
